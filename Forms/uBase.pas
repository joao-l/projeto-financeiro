...
begin
  ...
  begin
    ...
    // Validar é um componente que verifica se todos os campos do formulário estão preenchidos.
    // Ex.: Caso possua algum valor no formulario não preenchido o método do componente irar
    // retornar false e não sera possivel realizar o 'Post'
    ...
    begin
      ...
      // Verifica se o status do DataSet se é para inserção
      if DS.DataSet.State in [dsInsert] then
      begin
        try

          // Válida tabela títulos p/ que sempre que crie um novo registro do mesmo
      ...
      // Caso o DataSet não esteja no status p/ inserção(dsInsert)
      // o mesmo então sera para edição.
        try
          DS.DataSet.Post;
          (DS.DataSet as TClientDataSet).ApplyUpdates(0);
  ...
  if Tipo = 'Cancel' then
  begin
    // Caso o usuario cancele alguma operação.
...
begin
  // Ação parao o botão 'Apagar'
  Acao('Delete');
end;

procedure TFrmBase.ACEditarExecute(Sender: TObject);
begin
  // Ação parao o botão 'Editar'
  Acao('Edit');
end;

procedure TFrmBase.ACNovoExecute(Sender: TObject);
begin
  // Ação parao o botão 'Novo'
  Acao('New');
end;

procedure TFrmBase.ACSairExecute(Sender: TObject);
begin
  // Ação parao o botão 'Sair'
  Close;
end;

procedure TFrmBase.BtnCancelarClick(Sender: TObject);
begin
  // Ação parao o botão 'Cancelar'
  Acao('Cancel');
end;

procedure TFrmBase.BtnSalvarClick(Sender: TObject);
begin
  // Ação parao o botão 'Salvar'
  Acao('Save');
end;

procedure TFrmBase.cbBuscaChange(Sender: TObject);
begin
  // Função do campo de 'Busca', no qual a cada vez que o usuário selecionar
  // uma opção p/ pesquisa (ex. de busca de nome p/ busca por cpf)
  // o campo 'edtValor' sera limpado, caso o mesmo já
...
begin
  ...
  // Aqui é feita a pesquisa, no qual passamos o DataSet como parametro
...
begin
  // Ao fechar o formulario o DataSet é encerrado.
  FechaDataSet(DS);
end;
...
var
  I: Integer;
begin
  { Função que preenche o ComboBox do campo de Busca }

  // Condição que passa por todos os campos do DataSet
  for I := 0 to DS.DataSet.Fields.Count - 1 do
  begin
    // Condição que irar filtrar os campos que serão adicionados no ComboBox p/ Pesquisa
    // Ex.: O campo CPF é do tipo string (ftString) então o mesmo será adicionado ao ComboBox
    // p/ pesquisa
    if (DS.DataSet.Fields[I].DataType in [ftString]) or (DS.DataSet.Fields[I].DataType in [ftFMTBcd]) then
    begin

      // Condição p/ que não seja adicionado ao ComboBox a possibilidade de pesquisar por 'Títular'
      if DS.DataSet.Fields[I].DisplayName <> 'Títular' then
        cbBusca.Items.AddObject(DS.DataSet.Fields[I].DisplayName, TObject(I));
    end;
  end;
  // Setamos o ComboBox com o primeiro item, p/ que o mesmo não fique em Branco ao abrir o formulário
  cbBusca.ItemIndex := 0;
